...
router.get('/posts/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await client.query(`SELECT * FROM posts WHERE id = ${id}`, (err, result) => {
      if (err) { console.error('Error executing query', err); } else { res.send(result.rows); }
    });
  } catch (err) {
    console.error('Error connecting to PostgreSQL database', err);
...
router.post('/posts', async (req, res) => {
  ...
  connect().then(() => { console.log('Connected to PostgreSQL database');
    client.query(addPost, values, (err, result) => {
...
// Create a new table
router.post('/create_table/:name', async (req, res) => {
...
// Delete the table
router.delete('/delete_table/:name', async (req, res) => {
  const { name } = req.params;
...
router.post('/users', async (req, res) => {
  ...
  connect().then(() => { console.log('Connected to PostgreSQL database');
    client.query(addUser, values, (err, result) => {
      if (err) { console.error('Error executing query', err); } else { console.log('Data was inserted successfully.'); res.send(result); }
      // client.end();
    });
  })
